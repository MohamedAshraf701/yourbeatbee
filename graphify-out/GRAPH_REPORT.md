# Graph Report - musicai  (2026-08-15)

## Corpus Check
- 708 files · ~2,326,141 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 9687 nodes · 14439 edges · 233 communities detected
- Extraction: 82% EXTRACTED · 18% INFERRED · 0% AMBIGUOUS · INFERRED: 2621 edges (avg confidence: 0.71)
...
2. `LLMHandler` - 104 edges
3. `AceStepConfig` - 102 edges
4. `AceStepConfig` - 102 edges
5. `MomentumBuffer` - 76 edges
6. `AceStepHandler` - 69 edges
7. `GenerationParams` - 61 edges
8. `_Host` - 56 edges
9. `MetadataConstrainedLogitsProcessor` - 52 edges
10. `InitServiceMixinTests` - 50 edges

## Surprising Connections (you probably didn't know these)
- `Page()` --calls--> `listSongs()`  [INFERRED]
  app/page.tsx → lib/songs.ts
- `GET()` --calls--> `listSongs()`  [INFERRED]
  app/api/songs/route.ts → lib/songs.ts
- `GET()` --calls--> `getSettings()`  [INFERRED]
  app/api/setup/settings/route.ts → lib/settings.ts
- `GET()` --calls--> `readSetupStatus()`  [INFERRED]
  app/api/setup/install/route.ts → lib/setup-install.ts
- `GET()` --calls--> `readSetupStatus()`  [INFERRED]
  app/api/setup/status/route.ts → lib/setup-install.ts

## Communities

### Community 0 - "Community 0"
Cohesion: 0.01
Nodes (419): AceStepConfig, r"""     This is the configuration class to store the configuration of a [`AceSt, AceStepAttention, AceStepAudioTokenizer, AceStepConditionEncoder, AceStepConditionGenerationModel, AceStepDiTLayer, AceStepDiTModel (+411 more)

### Community 1 - "Community 1"
Cohesion: 0.01
Nodes (402): generate_examples(), Generate examples using LM and save to JSON files          Args:         num_exa, load_example_config(), Load configuration from example JSON file, applying CLI overrides., Run a single tier test with the given configuration.      Args:         sim_gb:, _run_single_tier_test(), main(), AudioSaver (+394 more)

### Community 2 - "Community 2"
Cohesion: 0.01
Nodes (344): _cleanup_gpu(), _dispatch(), _has_subcommand(), main(), Entry point for Side-Step training CLI.      When invoked with a subcommand (``p, Run the two-pass preprocessing pipeline., Run gradient sensitivity estimation., Check if sys.argv contains a recognized subcommand or --help. (+336 more)

### Community 3 - "Community 3"
Cohesion: 0.01
Nodes (262): auto_detect_backend(), build_parser(), _cleanup_dir(), _cleanup_handlers(), _get_vram_info_str(), initialize_handlers(), load_env_config(), main() (+254 more)

### Community 4 - "Community 4"
Cohesion: 0.01
Nodes (164): MetadataConstrainedLogitsProcessor, Extract genres from the user's caption that match entries in the vocabulary., Recursively collect all complete genres under a trie node.         Limited depth, Precompute mapping from characters to token IDs and token decoded texts., Initialize the constrained logits processor.                  This processor sho, Check if genres vocab file has been updated and reload if necessary., Get the trie node for a given prefix.         Returns None if the prefix is not, Check if the given text is a complete genre in the vocabulary. (+156 more)

### Community 5 - "Community 5"
Cohesion: 0.01
Nodes (196): _configure_cpu_threads(), configure_cpu_threads_if_needed(), debug_end(), debug_end_for(), debug_end_verbose_for(), debug_log(), debug_log_for(), debug_log_verbose_for() (+188 more)

### Community 6 - "Community 6"
Cohesion: 0.01
Nodes (196): create_help_button(), MdToHtmlTests, Markdown conversion unit tests for help-content rendering., Tests for the lightweight Markdown-to-HTML converter., ## headings become <h3> tags., ### headings become <h4> tags., **bold** becomes <strong>., *italic* becomes <em>. (+188 more)
...
Cohesion: 0.01
Nodes (152): LyricAlignmentCommonMixin, Shared helpers for lyric alignment and scoring mixins., Build the standard timestamp error payload., Build the standard lyric-score error payload., Provide shared data preparation helpers for lyric alignment methods., Load lyric alignment layers config from model config if available.          Call, Return caller config when provided, otherwise host default config., Move alignment tensors to the handler runtime device and dtype. (+144 more)

### Community 10 - "Community 10"
Cohesion: 0.02
Nodes (203): detect_base_model(), fuzzy_search(), get_base_defaults(), ModelInfo, pick_model(), prompt_base_model(), Model discovery and selection for Side-Step.  Scans a checkpoint directory for m, Infer which **training-schedule** base variant a model descends from.      Uses (+195 more)
...
Cohesion: 0.01
Nodes (136): create_openrouter_router(), Create OpenRouter-compatible API router.      Args:         app_state_getter: Ca, configure_api_routes(), Application route and middleware registration helpers for API server., Configure middleware, compatibility router, and all API route modules., Unit tests for API route setup helper wiring., Behavior tests for middleware and route registration orchestration., RouteSetupTests (+128 more)

### Community 13 - "Community 13"
Cohesion: 0.02
Nodes (138): Gradio UI Event Handlers Module Main entry point for setting up all event handle, Setup generation/results event wiring for the Gradio UI.      Args:         demo, Setup event handlers for the training tab (dataset builder and LoRA training), setup_event_handlers(), setup_training_event_handlers(), I18nHelpKeysTests, I18n key-coverage tests for help-content strings., Verify that all language files contain the required help.* keys. (+130 more)

### Community 14 - "Community 14"
Cohesion: 0.02
Nodes (101): _Host, InitServiceMixinTests, _load_init_service_module(), Unit tests for InitServiceMixin behavior and initialization orchestration., It treats backend aliases like ``cuda`` and ``cuda:0`` as equivalent., It defers to self.dtype for VAE when ROCm is active., It propagates float16 VAE dtype when self.dtype is float16 on ROCm., It returns bfloat16 for VAE on standard CUDA (non-ROCm) devices. (+93 more)
...
Cohesion: 0.02
Nodes (44): SettingsPanel(), onVisible(), ping(), ThemeProvider(), formatTime(), handleOpenChange(), onImport(), pickMimeType() (+36 more)

### Community 18 - "Community 18"
Cohesion: 0.02
Nodes (93): ApiAutoDurationTests, build_generation_setup must pass auto-sentinel for LM auto-calculation., When audio_duration is None, params.duration should be the auto-sentinel (-1)., Zero duration should get the auto-sentinel for LM auto-calculation., Negative duration (-1) should pass as auto-sentinel for LM auto-calculation., When audio_duration is explicitly set, it should be preserved., maybe_handle_analysis_only_modes(), Analysis-mode runtime helpers for API job generation. (+85 more)
...
Cohesion: 0.03
Nodes (101): _CapturingModel, FlowEditPipelineConditionForwardingTests, Pipeline-level tests for ``flow_edit_pipeline.flowedit_generate_audio``.  Covers, Stub model whose ``prepare_condition`` records the kwargs it saw., Regression for codex P2 round-1 finding: hints must reach prepare_condition., FlowEditSamplingLoopContractTests, Loop-contract tests for ``flowedit_sampling_loop`` (#1156).  Uses the determinis, Regression for codex P2 round-2 finding.          Pre-fix the inner ``n_avg`` lo (+93 more)
...
Cohesion: 0.05
Nodes (84): Page(), GET(), Studio(), GET(), POST(), POST(), GET(), GET() (+76 more)
...
Cohesion: 0.04
Nodes (37): LoRA management implementation modules., collect_adapter_names(), get_peft_initial_scale(), is_lora_like_module(), is_peft_factor_set_scale_module(), Pure LoRA introspection helpers reusable across runtimes and UIs., Detect modules where set_scale(adapter, factor) semantics are expected., Return PEFT LoRA baseline scale (alpha/r or alpha/sqrt(r)) for adapter. (+29 more)
...
Cohesion: 0.04
Nodes (52): _add_cors_middleware(), _add_cors_middleware_post_launch(), _close_result_cache(), create_random_sample(), format_input(), _get_api_key(), _get_project_root(), get_result() (+44 more)

### Community 41 - "Community 41"
Cohesion: 0.04
Nodes (48): _call_uses_keyword_function(), DecompositionContractGenerationTests, _event_chain_root(), _is_generation_mode_change_call(), Generation-focused decomposition contract tests., Mode switches should not overwrite manually tuned DCW controls., Run wiring should call clear, generate stream, and background pre-generation hel, Batch navigation wiring should call previous/next/background results helpers. (+40 more)

### Community 42 - "Community 42"
Cohesion: 0.04
Nodes (40): _add_common_training_args(), _add_device_args(), _add_estimation_args(), _add_fixed_args(), _add_model_args(), build_fixed_standalone_parser(), build_root_parser(), Argparse construction for ACE-Step Training V2 CLI.  Contains ``build_root_parse (+32 more)

### Community 43 - "Community 43"
Cohesion: 0.05
Nodes (33): _build_restore_js(), get_user_preferences_head(), _load_preferences_script(), Frontend user-preference persistence helpers for the Gradio UI.  Save side:  A `, Map JS restore results into Gradio output values.      The JS function reads loc, Attach a ``demo.load()`` handler that restores saved preferences.      Must be c, Load the external save-preferences JavaScript asset., Return Gradio head HTML that injects save-side preference persistence. (+25 more)

### Community 44 - "Community 44"
Cohesion: 0.05
Nodes (37): Chunk-level VAE encode helpers for tiled audio-to-latent conversion., Standard tiled encode keeping all data on GPU., Tiled encode that offloads latent chunks to CPU immediately., Implement chunked encode strategies for GPU and CPU-offload modes., VaeEncodeChunksMixin, VAE encode orchestration helpers for tiled audio-to-latent conversion., High-level VAE encode entrypoints and runtime chunk policy., Encode audio to latents using overlap-discard tiling.          Args: (+29 more)
...
Cohesion: 0.06
Nodes (31): apply_user_metadata_overrides(), build_fallback_caption(), build_format_request_intent(), caption_needs_retry(), build_localized_fallback_caption(), Localized fallback-caption text for external LM formatting helpers., Return localized fallback caption prose for supported language codes., Return the supported fallback-caption language key for user metadata. (+23 more)

### Community 54 - "Community 54"
Cohesion: 0.06
Nodes (27): GPUMonitor, Cached GPU / VRAM monitor for the live training display.  Wraps ``gpu_utils`` wi, Return peak allocated VRAM in MiB (CUDA only)., A point-in-time GPU memory reading., Cached GPU VRAM monitor.      Queries the device at most every ``interval`` seco, Cache device name and total VRAM (these don't change)., Return the latest VRAM snapshot, potentially cached., VRAMSnapshot (+19 more)

### Community 55 - "Community 55"
Cohesion: 0.08
Nodes (31): _cache_key(), external_lm_model_cache_path(), invalidate_cached_external_models(), _load_cache_payload(), load_cached_external_models(), _matches_invalidation(), Cached model-list helpers for external LM provider discovery., Load the full cache payload from disk when available. (+23 more)
...
Cohesion: 0.06
Nodes (22): ConditioningTargetMixin, ConditioningTargetMixin, Target-latent preparation helpers for handler batch conditioning., Mixin containing target-audio to latent preparation helpers.      Depends on hos, Normalize cached repaint source latents to ``[B, T, C]``., Trim or silence-pad cached source latents to the source-audio length., Return a silence-latent slice of exactly ``length`` frames.          When the pr, Encode target audio/codes to latents and pad batch tensors. (+14 more)
...
Cohesion: 0.11
Nodes (9): is_tensorboard_available(), Extended TensorBoard Logging for ACE-Step Training V2  Provides helpers for:, Log estimation gradient sensitivity scores., Log weight histograms for trainable parameters., Return True if tensorboard is importable., Wrapper around ``SummaryWriter`` with training-specific helpers.      If TensorB, Log a single scalar value., Compute and log the L2 gradient norm of every named parameter.          Only par (+1 more)

### Community 83 - "Community 83"
Cohesion: 0.18
Nodes (19): applyPreferredVolume(), applyPreferredVolumeToSlider(), beginStartupResync(), clampVolume(), discoverRoots(), forEachAudioPlayer(), forEachVolumeSlider(), handleDocumentClick() (+11 more)

### Community 84 - "Community 84"
Cohesion: 0.16
Nodes (19): _latent(), Tests for ``acestep.models.common.dcw_correction``.  These tests are designed to, test_corrector_disabled_is_identity(), test_corrector_double_mode_complementary_schedule(), test_corrector_double_mode_high_only_at_t_zero(), test_corrector_double_mode_low_only_at_t_one(), test_corrector_high_mode_is_identity_at_t_one(), test_corrector_high_mode_max_at_t_zero() (+11 more)

### Community 85 - "Community 85"
Cohesion: 0.09
Nodes (12): PassphraseStoreTests, Tests for cross-platform runtime passphrase storage helpers., Backends without ``KeyringError`` should still degrade cleanly on read failure., Whitespace-only secret service env vars should fall back to the defaults., A hung secret-tool call should degrade cleanly to the python keyring fallback., Verify env, file, and keyring passphrase fallback behavior., Environment passphrase should take precedence., Environment passphrases should remain byte-for-byte intact. (+4 more)

### Community 86 - "Community 86"
Cohesion: 0.18
Nodes (17): get_headers(), handle_response(), ACE-Step OpenRouter API 客户端测试代码  使用 requests 库测试 API 的各个端点和功能模式。  Usage:     pyt, 测试自然语言模式 (Sample Mode), 测试纯歌词模式 (Lyrics-Only Mode), 测试纯音乐模式 (Instrumental Mode), 测试流式响应模式 (Streaming Mode), 从 base64 data URL 保存音频文件 (+9 more)

### Community 87 - "Community 87"
Cohesion: 0.1
Nodes (11): AuthHelpersTests, Unit tests for API-key authentication helpers., Behavior tests for API-key state and token/header validation helpers., Reset auth state before each test., Reset auth state after each test., Token verification should allow requests when no API key is configured., Body ai_token should be accepted when it matches configured API key., Body ai_token mismatch should return the legacy 401 detail message. (+3 more)

### Community 88 - "Community 88"
Cohesion: 0.15
Nodes (19): _load_keyring_module(), _load_passphrase_from_keyring(), _load_passphrase_from_secret_tool(), Cross-platform runtime passphrase storage helpers., Write passphrase to Linux libsecret when ``secret-tool`` exists., Read passphrase from Python keyring when available., Write passphrase to Python keyring when available., Return normalized secret service coordinates with safe fallback defaults. (+11 more)

### Community 89 - "Community 89"
Cohesion: 0.12
Nodes (13): _Host, _load_service_generate_module(), Unit tests for extracted service-generation orchestration.  This test module loa, Create and return a recording helper stub for known mixin dependencies., Verify service-generate entrypoint orchestration after extraction., It executes helper stages in order and returns attached final outputs., It forwards runtime tuning controls to downstream helper invocations., Load ``acestep.core.generation.handler.service_generate`` from disk.      Inputs (+5 more)
...
Cohesion: 0.11
Nodes (5): Tests for ``acestep.models.mlx.dcw_correction_mlx``.  The test module is import-, Selecting a non-Haar wavelet should produce a different output — i.e.     the to, MLX non-Haar output must match the CUDA/CPU torch path bit-for-bit     (minus fl, test_apply_mlx_dcw_matches_torch_path_exactly(), test_apply_mlx_dcw_non_haar_differs_from_haar()

### Community 94 - "Community 94"
Cohesion: 0.17
Nodes (10): Model selection helpers for per-job DiT handler routing., Resolve the handler/model name for a generation job request.      Args:, select_generation_handler(), JobModelSelectionTests, Unit tests for per-job model handler selection helper., Behavior tests for model routing and fallback logging., No requested model should return primary handler/model., Requested model should route to second handler when initialized and matched. (+2 more)

### Community 95 - "Community 95"
Cohesion: 0.21
Nodes (16): check_ace_step_env(), check_cuda_availability(), check_nvidia_setup(), check_pytorch(), check_rocm_setup(), main(), print_recommendations(), print_section() (+8 more)

### Community 96 - "Community 96"
Cohesion: 0.17
Nodes (11): _make_handler(), _mock_gpu_config(), Unit tests for enforce_eager logic when flash_attn is unavailable.  Regression t, Jetson GPUs must still use enforce_eager=True even if flash_attn is detectable., Return a bare LLMHandler with no models loaded., Return a minimal GPU config mock., Verify that enforce_eager=True is set when flash_attn is not installed., Call handler.initialize() with all heavy operations mocked.          Args: (+3 more)

### Community 97 - "Community 97"
Cohesion: 0.17
Nodes (9): text2music must never process src_audio., text2music must ignore src_audio even when audio codes are empty., cover task should process src_audio when no codes provided., cover task should ignore src_audio when audio codes exist., repaint task should process src_audio., When src_audio is None, no processing should occur for any task., Verify handler-level defence: text2music skips process_src_audio., Reproduce the handler's src_audio processing logic.          This mirrors the br (+1 more)

### Community 98 - "Community 98"
Cohesion: 0.14
Nodes (11): maybe_compile(), Compatibility utilities for optional dependencies.  Provides graceful fallbacks, Apply ``torch.compile`` only when its backend (Triton) is available.      Drop-i, MaybeCompileTests, Unit tests for the ``maybe_compile`` conditional compilation decorator., Verify maybe_compile compiles or skips based on Triton availability., When Triton is available, the function should be passed to torch.compile., When Triton is absent, the original function should be returned unmodified. (+3 more)

### Community 99 - "Community 99"
Cohesion: 0.12
Nodes (9): ModelDownloadTests, Unit tests for API model auto-download helper functions., Ensure helper should fallback to HuggingFace when ModelScope fails., Behavior tests for extracted model download helper module., Unified repo downloads should target local_dir directly., ModelScope download should retry with cache_dir for compatibility., Ensure helper should return existing model path without download attempts., Ensure helper should honor explicit HuggingFace preference. (+1 more)

### Community 100 - "Community 100"
Cohesion: 0.25
Nodes (15): get_memory_stats(), main(), measure_cuda_context(), profile_dit(), profile_lm(), profile_text_encoder(), profile_vae(), Profile DiT model memory consumption. (+7 more)

### Community 101 - "Community 101"
Cohesion: 0.15
Nodes (11): _DitHost, _load_handler_module(), MlxDitInitMixinTests, Unit tests for extracted MLX DiT initialization mixin., Load handler mixin module directly from file path., Minimal host exposing DiT init state used by tests., Initialize deterministic model/config placeholders., Behavior tests for extracted ``MlxDitInitMixin``. (+3 more)

### Community 102 - "Community 102"
Cohesion: 0.18
Nodes (9): _load_progress_mixin(), Unit tests for ProgressMixin._get_project_root path resolution., Load ProgressMixin directly without importing the full handler package., Tests for ProgressMixin._get_project_root()., _get_project_root returns the current working directory when no env var is set., _get_project_root returns the ACESTEP_PROJECT_ROOT env var path when set., ACESTEP_PROJECT_ROOT overrides the current working directory., _get_project_root must not return a path derived from __file__ (site-packages fi (+1 more)

### Community 103 - "Community 103"
Cohesion: 0.16
Nodes (9): parse_quantization_arg(), Argument parsing helpers shared by ACE-Step CLI entrypoints., Parse ``--quantization`` values from CLI input.      Args:         value: Raw CL, ParseQuantizationArgTests, Unit tests for CLI argument parsing helpers., Behavior tests for quantization CLI parsing., It treats ``none`` aliases as disabled quantization., It returns supported quantization values in canonical form. (+1 more)

### Community 104 - "Community 104"
Cohesion: 0.19
Nodes (8): PipelineStartupBackendTests, Unit tests for CLI startup LM backend resolution in the pipeline., Service mode should not re-enable vLLM on legacy CUDA hardware., API-mode env overrides should still resolve to the safe startup backend., Verify startup backend resolution respects legacy CUDA restrictions., Return a representative legacy-CUDA GPU configuration., Run ``main`` with heavy dependencies stubbed and capture startup state., Legacy CUDA startup should override an explicit CLI vLLM request.

### Community 105 - "Community 105"
Cohesion: 0.19
Nodes (8): LauncherLegacyTorchFixTests, Tests for legacy NVIDIA torch compatibility launcher guards., Ensure launcher compatibility fixes are narrowly scoped and opt-out capable., Read a launcher script from the project root., Linux Gradio launcher should call shared Python compatibility probe., Linux API launcher should call shared Python compatibility probe., Windows Gradio launcher should call shared Python compatibility probe., Windows API launcher should call shared Python compatibility probe.

### Community 106 - "Community 106"
Cohesion: 0.16
Nodes (9): Task-type helpers for Gradio generation events., Resolve the Remix ``no_fsq`` checkbox into the backend task type.      Args:, resolve_no_fsq_task_type(), GenerationTaskTypeTests, Unit tests for Gradio generation task-type helpers., Checking no_fsq in Remix should select the non-FSQ backend task., Unchecked Remix should keep the default FSQ cover behavior., Hidden checked state should not alter other generation modes. (+1 more)

### Community 107 - "Community 107"
Cohesion: 0.16
Nodes (9): Async job execution runtime helper for API queue workers., Execute one queued job end-to-end with success/failure cache updates.      Args:, run_one_job_runtime(), JobExecutionRuntimeTests, Unit tests for async job execution runtime helper., Integration-style check with real run_in_executor and callback wiring., Behavior tests for queue-worker runtime execution orchestration., Success path should mark succeeded and write terminal cache success status. (+1 more)

### Community 108 - "Community 108"
Cohesion: 0.2
Nodes (9): get_vllm_preflight_warning(), _has_working_triton_installation(), Compatibility helpers for optional 5Hz LM backends., Return a user-facing warning when vLLM should be skipped before initialization., Return whether the Triton modules required by nano-vllm import cleanly., Verify backend preflight guards for optional Triton-dependent paths., Windows CUDA should skip vLLM when Triton imports are unavailable., Non-Windows or non-CUDA setups should not be blocked by the Triton preflight. (+1 more)

### Community 109 - "Community 109"
Cohesion: 0.24
Nodes (11): is_cjk(), process_folder(), OpenAI Whisper transcription service.  Transcribe audio to lyrics text using Ope, Transcribe audio and save plain lyrics text to file.      Args:         audio_pa, Transcribe all audio files in a folder.      Args:         input_dir: Directory, Convert word-level timestamps to plain lyrics text.      Uses timestamps to dete, Transcribe audio using OpenAI Whisper API.      Args:         audio_path: Path t, smart_join() (+3 more)

### Community 110 - "Community 110"
Cohesion: 0.24
Nodes (11): is_cjk(), process_folder(), ElevenLabs Scribe transcription service.  Transcribe audio to lyrics text using, Transcribe audio and save plain lyrics text to file.      Args:         audio_pa, Transcribe all audio files in a folder.      Args:         input_dir: Directory, Convert word-level timestamps to plain lyrics text.      Uses timestamps to dete, Transcribe audio using ElevenLabs Scribe API.      Args:         audio_path: Pat, smart_join() (+3 more)

### Community 111 - "Community 111"
Cohesion: 0.2
Nodes (9): discover_audio_files(), load_dataset_metadata(), load_sample_metadata(), Audio file discovery and metadata loading for preprocessing.  Reads both per-sam, Load the top-level ``metadata`` block from an ACE-Step dataset JSON.      Return, Select sample indices that should use genre instead of caption.      Mirrors ups, Discover audio files from a dataset JSON or by scanning a directory.      Resolu, Build a filename -> metadata mapping.      If *dataset_json* is provided, load i (+1 more)

### Community 112 - "Community 112"
Cohesion: 0.22
Nodes (6): install_rich_help(), Rich-enhanced argparse help formatter.  Replaces the default ``argparse.HelpForm, argparse help formatter that uses Rich for styled terminal output.      Falls ba, Override to apply Rich styling to the help output., Replace the formatter class on an existing parser and all subparsers., RichHelpFormatter

### Community 113 - "Community 113"
Cohesion: 0.25
Nodes (5): FacadeReExportTests, Unit tests for the results_handlers facade module.  Verifies that the thin re-ex, Ensure the facade re-exports all required public symbols., Every symbol that callers depend on must be importable from the facade., Each symbol should be either callable (function) or a constant (str/int).

### Community 114 - "Community 114"
Cohesion: 0.29
Nodes (6): _collect_nodes_missing_docstrings(), DocstringCoverageTests, Docstring coverage tests for decomposed event-wiring modules., Return qualified names for functions/classes missing docstrings., Ensure decomposed event-wiring modules keep full docstring coverage., Each target module and all nested defs/classes should have docstrings.

### Community 115 - "Community 115"
Cohesion: 0.25
Nodes (5): GenerationServiceWiringTests, Contract tests for generation service wiring., Verify key event hooks are present in generation service wiring., Service wiring should attach a change handler for language dropdown., Runtime language helper should exist for dropdown change wiring.

### Community 116 - "Community 116"
Cohesion: 0.32
Nodes (3): findInput(), readValue(), wireListeners()

### Community 117 - "Community 117"
Cohesion: 0.25
Nodes (7): API-key authentication helpers shared across HTTP route registration., Set the process-level API key used by auth verification helpers.      Args:, Validate request auth from body ``ai_token`` or Authorization header.      Args:, Validate Authorization header for endpoints that do not parse request body., set_api_key(), verify_api_key(), verify_token_from_request()
...
Cohesion: 0.38
Nodes (6): convert_and_load(), convert_vae_weights(), _fuse_weight_norm(), Convert PyTorch VAE weights and load them into an MLX VAE.      Args:         py, Fuse weight_norm parameters into a single weight tensor.      weight_norm decomp, Convert PyTorch AutoencoderOobleck weights to MLX format.      The function extr

### Community 120 - "Community 120"
Cohesion: 0.33
Nodes (3): Unit tests for the training_handlers facade.  Verifies that all expected public, Verify the facade re-exports all expected symbols., TestTrainingFacadeExports

### Community 121 - "Community 121"
Cohesion: 0.33
Nodes (4): GenerationServiceConfigRowsTests, Contract tests for generation service-config row builders., Verify service row builder contracts needed by the UI wiring., Language selector should remain interactive for runtime language selection.

### Community 122 - "Community 122"
Cohesion: 0.33
Nodes (4): GenerationTabOptionalControlsTests, Contract tests for generation optional-parameter controls., Verify optional controls start locked when Auto toggles are enabled., Optional controls should initialize with ``interactive=False``.

### Community 123 - "Community 123"
Cohesion: 0.47
Nodes (5): generate_fixtures(), generate_single_sample(), main(), Generate a single synthetic preprocessed sample.      All tensors are random (no, Generate *num_samples* synthetic .pt files + manifest.json.      Args:         o
...
## Knowledge Gaps
- **4134 isolated node(s):** `Require a readable reference file; convert WebM/Opus to WAV when needed.`, `Merge fields into shared heartbeat state and write immediately.`, `Exit when the studio UI has been gone long enough and no jobs are active.`, `Map studio job fields to ACE-Step GenerationParams values.  This module has no A`, `Remove gendered / register cues so reference audio can steer timbre.` (+4129 more)
...
- **Thin community `Community 154`** (2 nodes): `Helpers for optional external text-model configuration flows.`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 166`** (1 nodes): `Time a code section with GPU synchronization.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 168`** (1 nodes): `Replicate the audio_params building logic from inference.py.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 169`** (1 nodes): `Initialization should avoid nano-vllm when the Windows Triton preflight fails.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 170`** (1 nodes): `Initialization should still fall back when vLLM returns a Triton-specific error`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 172`** (1 nodes): `Check if MLX framework is available (Apple Silicon).          Delegates to the c`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 173`** (1 nodes): `Context manager to load a model to GPU and offload it back to CPU after use.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 174`** (1 nodes): `Post-process caption to remove YAML multi-line formatting.         Converts YAML`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 175`** (1 nodes): `When task_type is text2music, src_audio passed to handler must be None.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 176`** (1 nodes): `Load all i18n language JSON files for help-key assertions.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 177`** (1 nodes): `Concurrent store/get on dict fallback must not raise.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 178`** (1 nodes): `Reject conversion output that has no serialized audio-code tokens.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 179`** (1 nodes): `Pass valid audio codes through to LM understanding.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 180`** (1 nodes): `Verify that init_service_wrapper preserves current batch_size when provided.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 181`** (1 nodes): `Verify that init_service_wrapper uses default batch_size when current_batch_size`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 182`** (1 nodes): `Invalid audio upload should emit warning toast and clear component value.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 183`** (1 nodes): `Valid audio upload should pass through unchanged without warning.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 184`** (1 nodes): `Source-role validation should surface a source-specific toast message.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 185`** (1 nodes): `Successful sample creation should populate generation fields and mode switch.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 186`** (1 nodes): `Caption formatting should return lm-not-initialized status when LM is unavailabl`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 187`** (1 nodes): `Lyrics-only formatting should strip wrapper quotes from returned lyrics.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 188`** (1 nodes): `Full sample formatting should return 9 outputs with language at index 5.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 189`** (1 nodes): `When JSON has thinking=True AND non-empty audio_codes, think should be False.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 190`** (1 nodes): `When JSON has thinking=True AND empty audio_codes, think stays True.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 191`** (1 nodes): `Uppercase MP3 metadata should be normalized to the UI-compatible values.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 192`** (1 nodes): `Invalid MP3 metadata should fall back to the supported defaults.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 193`** (1 nodes): `Storing a new batch should move CUDA tensors to CPU, freeing VRAM.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 194`** (1 nodes): `Return kwargs for direct assertion in tests.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 195`** (1 nodes): `init_service_wrapper must NOT pass the checkpoint dropdown value as project_root`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 196`** (1 nodes): `The project_root passed to initialize_service should be the parent of checkpoint`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 197`** (1 nodes): `Calling init_service_wrapper with init_llm=False must not overwrite llm_handler.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 198`** (1 nodes): `When init_llm=True and device='auto', initialize() must be called with 'auto' de`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 199`** (1 nodes): `Legacy CUDA restrictions should override a requested vllm backend.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 201`** (1 nodes): `Sample tokens from logits with optional top-k and top-p filtering.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 202`** (1 nodes): `Keep only adapter-agnostic targets when no adapter names are discoverable.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 203`** (1 nodes): `Generate music latents and metadata from text/audio conditioning inputs.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 204`** (1 nodes): `Inject a fallback ``__len__`` implementation for torch.compile introspection.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 205`** (1 nodes): `Format initialize_service status output for UI/API consumers.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 206`** (1 nodes): `Generate LRC timestamps by aligning decoder cross-attention to lyric tokens.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 207`** (1 nodes): `Return a torchao quantization config object for the requested mode.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 208`** (1 nodes): `It returns None (pass) when offload_to_cpu is True, regardless of free VRAM.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 209`** (1 nodes): `It returns error payload when offload is off and free VRAM is insufficient.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 210`** (1 nodes): `It returns None when offload is off but free VRAM exceeds estimate.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 211`** (1 nodes): `It returns None when CUDA is not available (CPU/MPS/XPU).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 212`** (1 nodes): `Sync model-side python files when checkpoint code metadata diverges.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 213`** (1 nodes): `AAC file should be readable via torchaudio fallback.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 214`** (1 nodes): `Build fake MLX VAE from PyTorch VAE placeholder.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 215`** (1 nodes): `Load a model to device for the context and offload back to CPU on exit.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 216`** (1 nodes): `Provide no-op model context manager for decode tests.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 217`** (1 nodes): `Calculate lyric alignment scores for pure-noise and regressed-latent inputs.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 218`** (1 nodes): `Convert numpy audio array to channels-first torch tensor.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 219`** (1 nodes): `Return the AffineQuantizedTensor class from torchao, or None if unavailable.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 220`** (1 nodes): `Return current process RSS in megabytes.          Uses ``/proc/self/statm`` on L`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 221`** (1 nodes): `Create from dictionary.          Handles backward compatibility for datasets wit`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 227`** (1 nodes): ```True`` if the corrector will actually modify the latent.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 230`** (1 nodes): `Repeat KV heads for GQA: [B, n_kv, L, D] -> [B, n_kv*n_rep, L, D].`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 231`** (1 nodes): `Construct from an AceStepConfig (transformers PretrainedConfig).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 232`** (1 nodes): `Construct from a PyTorch ``AutoencoderOobleck`` instance's config.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 234`** (1 nodes): `Worker orchestration should start tasks, process queue items, and stop cleanly.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 235`** (1 nodes): `Return module device string when available.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 236`** (1 nodes): `Move a module to a target device/dtype when possible.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 237`** (1 nodes): `Helper should initialize LLM with recommended model when auto mode is active.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 238`** (1 nodes): `Map legacy payload fields to current request field names.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 239`** (1 nodes): `Helper should skip model initialization when ACESTEP_NO_INIT resolves true.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 240`** (1 nodes): `Default behavior should skip model loading (lazy-load on first request).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 241`** (1 nodes): `Helper should initialize primary DiT and then call LLM startup helper         wh`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 242`** (1 nodes): `Helper should raise and persist init error when primary DiT fails.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 243`** (1 nodes): `Cache helper wrappers should forward parameters unchanged.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 244`** (1 nodes): `POST /v1/dataset/auto_label should accept retired chunk/batch aliases.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 245`** (1 nodes): `Bootstrap helper should initialize default app.state runtime fields.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 246`** (1 nodes): `Bootstrap helper should create extra handlers when secondary config paths exist.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 247`** (1 nodes): `Setup should add CORS, include router, and invoke all route registrars once.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 248`** (1 nodes): `Return the resolved TensorBoard log directory.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 249`** (1 nodes): `Load config from a JSON file.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 250`** (1 nodes): `Auto-detect GPU and load the decoder model.          Returns:             A tupl`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 251`** (1 nodes): `Construct handler + LoRATrainer and yield training updates.          Yields:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 252`** (1 nodes): ```True`` when VRAM monitoring is possible.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 253`** (1 nodes): `Ollama `/api/tags` responses should yield model names.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 254`** (1 nodes): `Discovery should not call urlopen for unsupported URL schemes.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `AceStepHandler` connect `Community 1` to `Community 3`, `Community 4`, `Community 5`, `Community 7`, `Community 8`, `Community 9`, `Community 14`, `Community 21`, `Community 23`, `Community 26`, `Community 27`, `Community 34`, `Community 37`, `Community 44`, `Community 47`, `Community 59`, `Community 61`, `Community 62`, `Community 70`, `Community 78`, `Community 89`, `Community 90`, `Community 101`?**
  _High betweenness centrality (0.273) - this node is a cross-community bridge._
- **Why does `LLMHandler` connect `Community 1` to `Community 96`, `Community 3`, `Community 4`, `Community 7`, `Community 108`, `Community 18`, `Community 21`, `Community 22`?**
  _High betweenness centrality (0.155) - this node is a cross-community bridge._
- **Why does `generate_with_progress()` connect `Community 1` to `Community 3`, `Community 69`, `Community 8`, `Community 106`, `Community 11`, `Community 19`, `Community 20`?**
...
- **Are the 54 inferred relationships involving `LLMHandler` (e.g. with `PreciseTimer` and `ChatMessage`) actually correct?**
  _`LLMHandler` has 54 INFERRED edges - model-reasoned connections that need verification._
- **Are the 98 inferred relationships involving `AceStepConfig` (e.g. with `TimestepEmbedding` and `AceStepAttention`) actually correct?**
  _`AceStepConfig` has 98 INFERRED edges - model-reasoned connections that need verification._
- **Are the 98 inferred relationships involving `AceStepConfig` (e.g. with `TimestepEmbedding` and `AceStepAttention`) actually correct?**
  _`AceStepConfig` has 98 INFERRED edges - model-reasoned connections that need verification._